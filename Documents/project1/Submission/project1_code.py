...
def new_extract_feature_vectors(file, dict):
    """
      Returns a bag-of-words representation of a text file, given a dictionary.
      The returned matrix is of shape (m, n), where the text file has m non-blank
      lines, and the dictionary has n entries. 
...
def averager(feature_matrix, labels):
    """
      ...
      Returns a length-n theta vector with non-zero theta_0. 
    """
...
def perceptron_alg(feature_matrix,labels):
    """
      Classifies a set of data points via the perceptron algorithm.
    """
    (nsamples, nfeatures) = feature_matrix.shape
    theta_vector = np.zeros([nfeatures])
    theta_0 = 0
    ...
    while (mistake and t<T):
        ...
        for i in xrange(0, nsamples):
            ...
            sample_features = feature_matrix[i, :]
            label = labels[i]
            perceptron_output = label*(theta_0 + np.dot(theta_vector, sample_features))
            if (perceptron_output <= 0):
                theta_vector = theta_vector + label*sample_features
                theta_0 = theta_0 + label
                mistake=True
...
def passive_aggressive_alg(feature_matrix,labels):
    """
      Classifies a set of data points via the passive aggressive algorithm.
    """
    (nsamples, nfeatures) = feature_matrix.shape
    theta_vector = np.zeros([nfeatures])
    theta_0 = 0
    ...
    while (mistake and t<T):
        ...
        for i in xrange(0, nsamples):
            ...
            perceptron_output = label*(theta_0 + np.dot(theta_vector, sample_features))
            if (perceptron_output <= 0):
                param = (1-perceptron_output)/(np.linalg.norm(sample_features)**2)
                
                theta_vector = theta_vector + label*sample_features*param
                theta_0 = theta_0 + label*param
                mistake=True
